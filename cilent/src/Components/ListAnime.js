...
const ListAnime = () => {
    ...
    const getAnime = async (query = "") => {
        try {
            ...
            const url = query ? `http://localhost:5000/anime/${query}` : "http://localhost:5000/anime";
    ...
    return (
        <>  
        <div className="my-3">
            <input type="text" className="form-control" value={searchQuery} onChange={handleSearchInputChange} placeholder="Search Anime" />
        </div>
            ...
            <tbody>
                {animeList.map((anime, index) => (
                    <tr key={anime.anime_id}>
                        ...
                        <td><button className=" btn btn-danger" onClick={() => deleteAnime(anime.anime_id)}>Delete</button></td>
                    </tr>
                ))}
            </tbody>    
            </table>
